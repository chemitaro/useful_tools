...
class TestingFlameworkEnum(Enum):
    PYTEST = TestingFlamework(
        ...
        usage=textwrap.dedent(
            ...
            - 類似のテストを行う場合はpytest.mark.parametrizeを使用してください。
            """
        ),
        command="pytest",
    )
...
TEST_CODE_CONVENTION_AND_KNOWLEDGE = textwrap.dedent(
    ...
    c. テストケースの設計:
        - 各テストケースは単一の概念や動作をテストする
        - 複数の概念を1つのテストケースにまとめない
        - 正常系、異常系、エッジケースを個別のテストケースとして作成
    ...
    a. アサーション:
        ...
        - 1つのテストケースに複数のアサーションを含める場合は、関連する検証に限定する
    ...
    ## 5. テストケースの粒度
    a. 単一責任の原則:
        - 各テストケースは1つの特定の動作や条件のみをテスト
        - 複数の概念や条件を1つのテストケースに混在させない

    b. 明確な目的:
        - テストケース名から何をテストしているか明確に理解できるようにする
        - テストの目的が不明確な場合は、さらに細分化することを検討

    c. 独立性:
        - 各テストケースは他のテストケースに依存せず、単独で実行可能であるべき

    d. 境界値と極端なケース:
        - 通常の入力、境界値、エッジケースを別々のテストケースとして実装

    e. 正常系と異常系の分離:
        - 正常な動作と例外的な動作を別々のテストケースで検証

    f. 可読性と保守性:
        - テストケースが長くなり過ぎる場合は、さらに細分化することを検討
        - 各テストケースは、その目的と検証内容が一目で理解できるようにする
    ...
    これらの規約とガイドラインに従うことで、読みやすく、保守しやすい、そして信頼性の高いテストコードを作成できます。各テストケースは適切な粒度を保ち、単一の概念をテストすることで、問題の特定と修正が容易になります。
    """
)
        ...
        messages=[
            LlmMessage(
                ...
                content=textwrap.dedent(
                    """
                    あなたは優秀なソフトウェアエンジニアで、コード分析のエキスパートです。提供された実装コードを詳細に分析し、テスト対象範囲の構造、振る舞い、および外部依存関係を正確に言語化する任務があります。以下の点に特に注意して分析を行ってください：
                    1. テスト対象範囲の特定: 提供されたコード全体の中から、指定されたテスト対象範囲を正確に識別する。
                    2. 構造の言語化: テスト対象範囲内のクラス、メソッド、関数の構造を簡潔かつ正確に記述する。
                    3. 振る舞いの言語化: 各要素（クラス、メソッド、関数）の主要な振る舞いや目的を、技術的な詳細を含めて説明する。
                    4. 入出力の記述: 各メソッド・関数の引数と戻り値の型、および期待される入出力の特徴を明確に示す。
                    5. 主要ロジックの説明: テスト対象範囲内の重要なロジックやアルゴリズムを、ステップバイステップで説明する。
                    6. 外部依存関係の詳細な分析:
                        - 使用されている外部ライブラリ、フレームワーク、モジュールを特定し、記録する。
                        - 外部ライブラリから継承またはインポートされているクラス、関数、メソッドを具体的に列挙する。
                        - これらの外部依存関係がテスト対象のコードにどのような影響を与えているかを説明する。
                    7. 状態変化の記述: オブジェクトの状態がどのように変化するか、特に重要な状態遷移を説明する。
                    8. エラー処理の概要: 主要なエラーケースや例外処理の概要を説明する。
                    分析結果は、後続のテスト設計ステップで利用されることを念頭に置き、明確で構造化された形式で提示してください。技術的な正確さと簡潔さのバランスを保ちつつ、テスト設計者が容易に理解できる言語で記述することが重要です。                    """
                ),
            ),
            LlmMessage(
                role="user",
                content=textwrap.dedent(
                    f"""
                    以下に提供される実装コードを分析し、指定されたテスト対象範囲に焦点を当てて詳細なレポートを作成してください。
                    ...
                    1. テスト対象範囲の概要
                        - テスト対象となるクラス、メソッド、関数の一覧
                        - これらの要素間の関係性の簡単な説明

                    2. 外部依存関係の詳細分析
                        - 使用されている外部ライブラリ、フレームワーク、モジュールの一覧
                        - 外部ライブラリから継承またはインポートされている具体的なクラス、関数、メソッド
                        - 各外部依存関係がテスト対象コードにどのような影響を与えているかの説明

                    3. 各要素の詳細分析
                    3.1 構造
                        - クラス、メソッド、関数の定義（シグネチャ）
                        - 主要な属性やプロパティの一覧
                        - 継承関係や実装しているインターフェースの説明

                    3.2 振る舞い
                        - 各要素の主要な目的と機能の説明
                        - 重要なロジックやアルゴリズムのステップバイステップの説明
                        - 外部依存関係の影響を受ける振る舞いの特定と説明
                    ...
                    3.5 エラー処理
                        - 主要なエラーケースや例外の概要
                        - エラー発生時の期待される振る舞い
                        - 外部依存関係に関連するエラーハンドリング

                    4. 全体の振る舞いのまとめ
                        - テスト対象範囲全体の主要な機能や目的の総括
                        - 個々の要素がどのように連携して全体の振る舞いを実現しているかの説明
                        - 外部依存関係が全体の振る舞いにどのように寄与しているかの説明

                    このレポートは、実装コードの構造、振る舞い、および外部依存関係を正確に言語化することに焦点を当ててください。テストケースの設計や具体的なテスト戦略の提案は含めないでください。分析結果は、後続のテスト設計プロセスで活用されることを念頭に置いて、明確で簡潔な言語で記述してください。特に、外部ライブラリやフレームワークの影響を詳細に分析し、それらがテスト対象コードの振る舞いにどのように影響を与えるかを明確に説明してください。
                    """
                ),
            ),
        ]
            ...
            messages=[
                LlmMessage(
                    ...
                    content=textwrap.dedent(
                        ...
                        出力形式：
                        - テストケースは自然言語で明確に構造化された文章を箇条書きで出力してください。
            ...
            messages=[
                ...
                LlmMessage(
                    ...
                    content=textwrap.dedent(
                        ...
                        - テストフレームワークの特性を活かすこと
                        - テストの可読性と保守性を確保すること
                        - テストケースは自然言語で明確に構造化された文章を箇条書きで出力すること。
